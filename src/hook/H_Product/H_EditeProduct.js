...
import { GET_ID_product, POST_product, PUT_product } from '../../redux/S_Product';


const H_EditeProduct = (idProd) => {
  const dispatch = useDispatch()
  const Product = useSelector(state => state.SliceProduct.product)
  useEffect(_ => {
    dispatch(GET_ID_product(idProd))
  }, [idProd])
  let items = []
  ...
  useEffect(_ => {
    ...
    setPriceAfterDiscount(items.priceAfterDiscount)
    setquantity(items.quantity)
    setDes(items.description)
    items.colors && items.colors.map(e => setColor([e]))
  },[items])
  ...
  const handleEdite = async (e) => {
    ...
    setisload(true)
    await dispatch(PUT_product(formdata));
    setisload(false)
  };
